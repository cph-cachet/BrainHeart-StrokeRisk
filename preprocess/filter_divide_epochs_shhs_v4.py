...
shh1_events_path = os.path.join(
    base_path, 'polysomnography', 'annotations-events-nsrr', 'shhs1')
# path to save the processes data
newpath = Path(
    '/work3/saima/shhs_processed/shhs1_processed_all_directories_V4')
...
cvd_summary = pd.read_csv(os.path.join(
    base_path, 'datasets', 'shhs-cvd-summary-dataset-0.21.0.csv'))
# %%
...
for sub in range(len(final_list_subjects)):  # --- run in loop in final
    ...
    if subejct_path == True:
        print('----------Sanity check-----------', subejct_path)
        print(f'subject {sub} already processed... skipping to next')

    if subejct_path == False:

        print('directory does not exist, therefore processing subject:', subject)
        # Load raw data
        raw = mne.io.read_raw_edf(os.path.join(
            shh1_data_path, f'shhs1-{subject}.edf'), preload=True, verbose=debug)
        print('Loading subject annotation in xml for subject:', subject)
        # Load events in xml file
        event_content = open(os.path.join(
            shh1_events_path, f'shhs1-{subject}-nsrr.xml')).read()
        # change xml format to ordered dictionart
        event_dict = xmltodict.parse(event_content)[
            'PSGAnnotation']['ScoredEvents']['ScoredEvent']
        # Ordered dictionary to dataframe
        event_df = pd.DataFrame(event_dict)
        event_df['EventConcept'] = event_df['EventConcept'].str.split(
            '|').str[0]
        event_df['EventType'] = event_df['EventType'].str.split('|').str[0]
        event_df['Start'] = pd.to_numeric(
            event_df['Start'], downcast='integer', errors='coerce')
        event_df['Duration'] = pd.to_numeric(
            event_df['Duration'], downcast='integer', errors='coerce')
        ...
        '''
        # 1: drop channels
        raw.drop_channels([item for item in raw.ch_names if item not in [
                          'EEG', 'EEG(sec)', 'EEG 2', 'EEG2', 'EEG sec', 'EEG(SEC)', 'ECG']], on_missing='warn')
        ...
        eeg_picks = [item for item in raw.ch_names if item in [
            'EEG', 'EEG(sec)', 'EEG 2', 'EEG2', 'EEG sec', 'EEG(SEC)']]
        # and pick ECG channels
        ecg_pick = [item for item in raw.ch_names if item in ['ECG']]
        # bandpass EEG
        raw.filter(l_freq=1, h_freq=45, picks=eeg_picks,
                   filter_length='auto', phase="zero-double", verbose=debug)
        # bandpass ECG
        raw.filter(l_freq=4, h_freq=45, picks='ECG',
                   filter_length='auto', phase="zero-double", verbose=debug)
        ...
        if 'REM sleep' in sleep_stages_df['EventConcept'].values:
            ...
            print("REM sleep stage not found")
