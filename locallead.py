...
class LocalLeadModel(nn.Module):
    def __init__(self, channels=3, num_classes=1):
        super(LocalLeadModel, self).__init__()
        self.is_train = True
        ...
        self.transformer = ConvTransformerBackbone(n_in=128,
                                                   ...
                                                   arch=(2, 2, 8),
                                                   use_abs_pe=False)
        self.linearN = nn.Sequential(nn.Linear(5*self.channels, 8),
                                     nn.ReLU(),
                                     nn.Linear(8, 1))
    ...
    def forward(self, x):
        x = self.blocks(x)
        x = F.upsample(x, size=(self.channels, self .max_len), mode='bilinear')
